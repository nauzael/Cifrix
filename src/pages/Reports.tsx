...
<div class="flex items-center gap-4"><div class="size-14 bg-blue-600 rounded-xl flex items-center justify-center text-white"><span class="material-symbols-outlined text-4xl">church</span></div><div><h1 class="text-2xl font-black tracking-tight text-slate-900 uppercase">${orgName}</h1><p class="text-slate-500 text-sm font-medium">NIT: ${taxId}</p><p class="text-slate-400 text-xs">${address}${phone ? ' • ' + phone : ''}${email ? ' • ' + email : ''}</p></div></div>
...
<div class="bg-slate-50 p-6 rounded-xl border border-dashed border-slate-300 mb-16">
<h4 class="text-xs font-black text-slate-600 uppercase mb-2 flex items-center gap-2">Declaración de Veracidad</h4>
<p class="text-xs text-slate-500 leading-relaxed">Certifico que la información financiera aquí presentada ha sido preparada y verificada bajo los estándares de auditoría interna de la organización.</p>
</div>
...
<div class="pdf-container max-w-4xl mx-auto bg-white shadow-2xl min-h-[29.7cm] p-12 border border-slate-200">
<header class="flex justify-between items-start border-b-2 border-slate-100 pb-8 mb-8"><div class="flex items-center gap-4"><div class="size-14 bg-blue-600 rounded-xl flex items-center justify-center text-white"><span class="material-symbols-outlined text-4xl">church</span></div><div><h1 class="text-2xl font-black tracking-tight text-slate-900 uppercase">${orgName}</h1><p class="text-slate-500 text-sm font-medium">NIT: ${taxId}</p><p class="text-slate-400 text-xs">${address}${phone ? ' • ' + phone : ''}${email ? ' • ' + email : ''}</p></div></div><div class="text-right"><div class="bg-blue-600/10 text-blue-600 px-3 py-1 rounded-full text-[10px] font-black tracking-widest uppercase mb-2 inline-block">Reporte Financiero</div><h2 class="text-xl font-bold text-slate-900 uppercase">Estado de Resultados</h2><p class="text-slate-500 text-sm">Periodo: ${nowDate}</p><p class="text-slate-400 text-xs mt-1 italic">Generado el ${nowDate} ${nowTime}</p></div></header>
...
.pdf-container { width: calc(8.5in - 1in); min-height: calc(11in - 1in); margin: 0 auto; background: #ffffff; page-break-after: auto; }
@media print {
  .no-print { display: none !important; }
  body { background: white !important; margin: 0 !important; padding: 0 !important; }
  .pdf-container { box-shadow: none !important; width: 100% !important; max-width: 100% !important; min-height: auto !important; border: none !important; background: #ffffff !important; margin: 0 !important; padding: 0.5in !important; }
...
`;
      let html = buildHtml(name);
      if (html.includes('<head>')) {
        html = html.replace('<head>', `<head>${basePrintStyles}`);
      }
      const address = org?.settings?.address || '';
      const phone = org?.settings?.phone || '';
      const email = org?.settings?.email || '';
      const contact = `${address}${phone ? ' • ' + phone : ''}${email ? ' • ' + email : ''}`;
      html = html.replace(/(<p class="text-slate-400 text-xs">)[\s\S]*?(<\/p>)/, `$1${contact}$2`);
      html = html.replace(/(<p class="text-slate-400 text-sm">)[\s\S]*?(<\/p>)/, `$1${contact}$2`);
      html = html.replace(/(NIT:\s*[^<]*<\/p>)(\s*<\/div>\s*<div class="text-right")/, `$1<p class="text-slate-400 text-xs">${contact}</p>$2`);
      html = html.replace(/(NIT:\s*[^<]*<\/p>)(\s*<p class="text-slate-400 text-xs uppercase tracking-wider">Libro Auxiliar Contable<\/p>)/, `$1<p class="text-slate-400 text-xs">${contact}</p>$2`);
